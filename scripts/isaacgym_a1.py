...
def main():
    ...
    robot_model = load_model(gym, sim, asset_root_path, robot_path, is_base_fixed=False)
    ...
    predictive_controller = ModelPredictiveController(LinearMpcConfig, A1Config)
    ...
    while not gym.query_viewer_has_closed(viewer):
        ...
        quat_base_imre=base_state_tensor[0, 3:7].cpu().numpy()
        quat_base_reim = np.array(
            [quat_base_imre[3], quat_base_imre[0], quat_base_imre[1], quat_base_imre[2]],
            dtype=np.float32
        )
        robot_data.update(
            pos_base=base_state_tensor[0, 0:3].cpu().numpy(),
            quat_base=quat_base_reim,
            lin_vel_base=base_state_tensor[0, 7:10].cpu().numpy(),
            ang_vel_base=base_state_tensor[0, 10:13].cpu().numpy(),
            q = joint_state_tensor[:, 0].cpu().numpy(),
            qdot = joint_state_tensor[:, 1].cpu().numpy()
        )
        ...
        torque_cmds_tensor = torch.tensor(torque_cmds, dtype=torch.float32, device='cuda:0')
